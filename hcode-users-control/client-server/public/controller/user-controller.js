            ...
            this.getPhoto(this.formAd).then(
                (content)=>{ 
                    ...
                    user.save().then(user =>{
                        ...
                        tr.innerHTML = `<tr>
                                        <td><img src=${user._photo} alt="User Image" class="img-circle img-sm"></td>
                                        <td>${user._name}</td>
                                        <td>${user._email}</td>
                                        <td>${(user._admin) ? 'Sim' : 'Não'}</td>
                                        <td>${dateFormater(user._register)}</td>
                                        <td>
    ...
    getValues(formId){//Carrega as informações contidas no form para serem salvas no onsubmit
        ...
        [...formId.elements].forEach(function(field, index) {
            ...
            if(['name','email','password'].indexOf(field.name) > -1 && !field.value){
                ...
                field.parentElement.querySelector(".help-block").style.display = 'block';
            ...
            if(['name','email','password'].indexOf(field.name) > -1 && field.value){
                ...
                field.parentElement.querySelector(".help-block").style.display = 'none';
